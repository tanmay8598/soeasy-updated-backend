...
const InvoiceUser = require("../invoice-models/userModel");
const ProfileModel = require("../invoice-models/ProfileModel");

const signin = async (req, res) => {
  const { email, password } = req.body; //Coming from formData

  try {
    const existingUser = await InvoiceUser.findOne({ email });
...
const signup = async (req, res) => {
  ...
  try {
    ...
    const result = await InvoiceUser.create({
      email,
...
const forgotPassword = (req, res) => {
  ...
  crypto.randomBytes(32, (err, buffer) => {
    ...
    InvoiceUser.findOne({ email: email }).then((user) => {
      if (!user) {
        return res
          .status(422)
...
module.exports = { resetPassword, forgotPassword, signin, signup };
